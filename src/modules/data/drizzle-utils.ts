 ...
 *
 * @example
 * // Check which tweet IDs have already been replied to
...
export async function queryWhereIn(params: {
  ...
  const { tableName, column, values, selectColumn } = params;
  ...
  logger.info(
    { tableName, column, valueCount: values.length },
...
async function ensureTable(tableName: string, data: Record<string, unknown>): Promise<void> {
  try {
    ...
    if (tableExists) {
      ...
      logger.info({ tableName, columns }, 'Created table dynamically');
    }
  } catch (error) {
...
export async function insertRecord(params: {
  ...
  logger.info({ tableName, fields: Object.keys(data) }, 'Inserting record');
  ...
  try {
    ...
    await ensureTable(tableName, normalizedData);
    ...
    logger.info({ tableName }, 'Record inserted successfully');
...
export async function updateRecord(params: {
  ...
  const { tableName, id, data, idColumn = 'id' } = params;

  logger.info({ tableName, id, fields: Object.keys(data) }, 'Updating record');
  ...
  try {
    ...
    logger.info({ tableName, id }, 'Record updated successfully');
...
export async function deleteRecord(params: {
  ...
  const { tableName, id, idColumn = 'id' } = params;

  logger.info({ tableName, id }, 'Deleting record');
  ...
  try {
    ...
    logger.info({ tableName, id, deleted }, 'Delete operation complete');
...
export async function queryRecords(params: {
  ...
  const { tableName, where, limit } = params;

  logger.info({ tableName, where, limit }, 'Querying records');
